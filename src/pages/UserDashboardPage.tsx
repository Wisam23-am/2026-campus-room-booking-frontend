...
export const UserDashboardPage: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        pending: data.filter((b) => b.status === 0).length,
        approved: data.filter((b) => b.status === 1).length,
        rejected: data.filter((b) => b.status === 2).length,
      });
    } catch (error) {
  ...
  return (
    <MainLayout userName="Alex Johnson" userRole="user">
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Welcome back, Alex! 👋</h1>
          <p className="text-slate-600 dark:text-slate-400 mt-1">Here's what's happening with your bookings</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-slate-500 dark:text-slate-400">No bookings yet. Create one to get started!</div>
          ) : (
            <div>
